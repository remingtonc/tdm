...
What is the big missing piece? The linking of Data Models to Operating Systems/Release versions. Given the SNMP legacy, that kind of release process data just simply doesn't seem to exist via an easily consumable API. For Cisco at least, we can get some level of insight via [supportlists](https://github.com/cisco/cisco-mibs/tree/main/supportlists), but these tend to lean towards being semi-hand-written HTML documents and not very machine consumable. Hence, business problem.
